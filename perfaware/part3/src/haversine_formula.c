...
static double ReferenceHaversine(double X0, double Y0, double X1, double Y1,
                                 double EarthRadius) {
  /* NOTE: This is not meant to be a "good" way to calculate the Haversine
     ...
     formula used in the real-world question on which these homework exercises
     are loosely based.
  */
  ...
  double a =
      Square(sin(dLat / 2.0)) + cos(lat1) * cos(lat2) * Square(sin(dLon / 2.0));
